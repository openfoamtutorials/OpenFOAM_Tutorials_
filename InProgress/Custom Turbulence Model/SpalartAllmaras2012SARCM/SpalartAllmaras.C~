...
{
    return 1.0-chi/(scalar(1) + chi*fv1);
...
{
    //New quantity fn (for negative-nuTilda Spalart-Allmaras model)
    volScalarField fn (	(16+pow3(chi))/(16-pow3(chi))	);
    return tmp<volScalarField>
    (
        //new volScalarField("DnuTildaEff", (nuTilda_ + nu())/sigmaNut_)
        new volScalarField("DnuTildaEff", (nuTilda_ + nu()*(pos(nuTilda_)+(1-pos(nuTilda_))*fn))/sigmaNut_)
    );
}
...
void SpalartAllmaras::correct()
{
    ...
    //New quantities for Modified Vorticity Stilda
    volScalarField Smag (	sqrt(2.0)*mag(skew(fvc::grad(U_)))	);
    volScalarField Sbar (	fv2(chi, fv1)*nuTilda_/sqr(kappa_*d_)	);
    
    //START SARCM: New quantities for SARCM
    volScalarField SSmag (	sqrt(2.0)*mag(symm(fvc::grad(U_)))	);
    volScalarField rStar ( SSmag/Smag );
    volScalarField rTilda (	(1-rStar)/(rStar*rStar)	);
    //Two sets of the three constants are included (respectively: the default, and another suggested by Qiang and Yong)
    //scalar Cr1=1,Cr2=12,Cr3=0.6;
    scalar Cr1=1,Cr2=2,Cr3=1;
    volScalarField fr1 (
	(1+Cr1)*2*rStar/(rStar+1)*(1-Cr3*atan(Cr2*rTilda))-Cr1	
    );
    //END SARCM: Now have fr1 from SARCM to use in the production term.

    const volScalarField Stilda
    (
    //New expression for Modified Vorticity Stilda
	Smag
	+pos(Sbar+0.7*Smag)*Sbar
	+(1-pos(Sbar+0.7*Smag))*Smag*(0.49*Smag+0.9*Sbar)/(-0.5*Smag-Sbar)
/*
...
*/
    ...
    (
     ...
     //DnuTildaEff now requires chi for Negative SA Model
      - fvm::laplacian(DnuTildaEff(chi), nuTilda_)
      - Cb2_/sigmaNut_*magSqr(fvc::grad(nuTilda_))
     ==
     //New expression as part of Negative SA Model
        fr1*Cb1_*nuTilda_*(pos(nuTilda_)*Stilda+(1-pos(nuTilda_))*-0.2*Smag)
      - fvm::Sp(Cw1_*nuTilda_/sqr(d_)*(pos(nuTilda_)*fw(Stilda)-(1-pos(nuTilda_))), nuTilda_)
