...
Function calculate_quarter_chord
	...
	*/
	dx = -0.25*target_chord*Cos(aoa);
	dy = 0.25*target_chord*Sin(aoa);
Return
...
Plane Surface(ce++) = {domain_loop, te_loop, le_loop, bl_loop };
domain_surf = ce;
...
Physical Surface("wing") = {ids[{6:13}]};//,baffle_surf};
